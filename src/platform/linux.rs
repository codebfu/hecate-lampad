...
const SERVICE_NAME: &str = "hecate-lampad";
const SERVICE_START_HINT: &str = "sudo systemctl enable --now hecate-lampad";
            ...
            .output()
        {
            Ok(output) => {
                ...
                let status = match state.as_str() {
                    ...
                    _ if output.status.code() == Some(4) => ServiceStatus::NotFound,
            ...
            Err(error) => ServiceReport {
                ...
                detail: Some(format!("systemctl unavailable: {error}")),
                runtime: None,
            },
        }
    }
